...
public class TagController {
	...
	// @RequestMapping(value = "/transactions_by_tag", method =
	// RequestMethod.POST)
	// public String showTransactionByTag(HttpSession session, Model model,
	...
	//
	// for (Entry<Integer, Transaction> entry :
	// user.getTransactions().entrySet()) { // Transactions
	...
	// }
	//
	// fromDateCheck = entry.getValue().getDate().after(fromDate) ||
	// entry.getValue().getDate().equals(fromDate);
	//
	// toDateCheck = entry.getValue().getDate().before(toDate) ||
	// entry.getValue().getDate().equals(toDate);
	//
	// // All check
	...
	// graphCompare.add(new KeyValue("Expense", String.valueOf(expence * -1),
	// null));
